...
Cvar::Range<Cvar::Cvar<int>> g_bot_defaultSkill( "g_bot_defaultSkill", "Default skill value bots will have when added", Cvar::NONE, 5, 1, 9 );
...
static int BotGetAimPredictionTime( gentity_t *self )
{
	...
	if ( G_Team( self ) == TEAM_ALIENS )
		time = ( 10 - self->botMind->botSkill.level ) * 200 * std::max( random(), 0.5f );
	else
		time = ( 10 - self->botMind->botSkill.level ) * 150 * std::max( random(), 0.5f );
...
static Cvar::Cvar<int> g_bot_crazyPounceMinHeight("g_bot_crazyPounceMinHeight", "minial height difference for bots to attack upwards.", Cvar::NONE, 100);
...
static bool BotAttackUpward( gentity_t *self )
{
	...
	if ( targetPos.z - ownPos.z > g_bot_crazyPounceMinHeight.Get() )
	{
